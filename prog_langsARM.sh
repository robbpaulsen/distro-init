...
cd &&
curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs -sSf | sh -s -- -y --profile default --no-modify-path --default-toolchain nightly toolchain nightly &&
...
# Commented out installation git clone https://github.com/via cargo, might be a better practice to source install
