...
cd &&
curl https://sh.rustup.rs -sSf | sh -s -- --profile default --no-modify-path --default-toolchain nightly rustup-managed toolchain nightly &&
...
sudo rm -rf /usr/local/go && sudo tar -C /usr/local -xzf go1.20.1* &&
